    ...
    public function budgets()
    {
        ...
        if ($this->search !== '') {
            ...
            $query->where(function ($q) use ($search) {
                $q->where('budgets.number', 'like', "%{$search}%")
                    ->orWhere('budgets.title', 'like', "%{$search}%")
        ...
        if ($this->status !== 'all') {
            $query->where('budgets.status', $this->status);
        }
...
<div>
    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
            <h1 class="text-2xl font-semibold tracking-tight text-slate-900">Orçamentos</h1>
            <p class="mt-1 text-sm text-slate-500">Gerencie os orçamentos enviados aos seus clientes.</p>
        </div>

        @can('create', Budget::class)
    ...
    <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
        <div class="grid gap-3 border-b border-slate-100 px-6 py-4 sm:grid-cols-2">
            ...
            <select wire:model.live="status"
                ...
                @foreach (BudgetStatus::cases() as $statusCase)
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($this->budgets as $budget)
                        <tr class="transition hover:bg-slate-50/70">
                            <td class="px-6 py-4">
                                <a href="{{ route('budgets.show', $budget) }}" class="block truncate font-medium text-slate-900 hover:text-indigo-600">
                                    #{{ $budget->number }}
                                </a>
                            </td>
                            <td class="px-6 py-4">
                                <p class="truncate text-slate-600">{{ $budget->client?->name ?? '—' }}</p>
                            </td>
                            <td class="hidden px-6 py-4 md:table-cell">
                                <p class="truncate text-slate-600">{{ $budget->title }}</p>
                            </td>
                            <td class="hidden px-6 py-4 sm:table-cell">
                                <div class="flex flex-wrap items-center gap-1.5">
                                    <span class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium ring-1 {{ $budget->status->badgeClasses() }}">
                                        {{ $budget->status->label() }}
                                    </span>
                                    @if ($budget->service_order_id !== null)
                                        <span class="inline-flex items-center rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-medium text-slate-500 ring-1 ring-slate-200">
                                            somente leitura
                                        </span>
                                    @endif
                                </div>
                            ...
                            <td class="px-6 py-4 text-right font-medium text-slate-900">
                                R$ {{ number_format((float) $budget->total, 2, ',', '.') }}
                            </td>
                            <td class="px-6 py-4">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-16 text-center">
                                <p class="text-sm font-medium text-slate-700">Nenhum orçamento encontrado</p>
                                <p class="mt-1 text-sm text-slate-500">Ajuste a busca ou comece criando um orçamento.</p>
