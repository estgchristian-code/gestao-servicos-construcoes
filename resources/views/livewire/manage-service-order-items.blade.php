...
new class extends Component {
    ...
    {
        return $this->order->items()->with('service')->orderBy('id')->get();
    }
    ...
    public function services()
    {
        return Service::query()
            ->forCompany($this->order->company_id)
    ...
    public function save(): void
    {
        ...
        if ($this->editingItemId !== null) {
            $item = $this->order->items()
                ->where('service_order_items.company_id', $this->order->company_id)
            ...
            $item->service_order_id = $this->order->id;
            $item->company_id = $this->order->company_id;
            $item->save();
...
<div>
    <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
        <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
            <div>
                <h2 class="text-sm font-semibold text-slate-900">Itens da ordem de serviço</h2>
                <p class="mt-0.5 text-xs text-slate-500">Os itens definem o valor total da OS.</p>
            </div>
            ...
            <div class="divide-y divide-slate-100">
                ...
                @empty
                    <div class="px-6 py-12 text-center">
                        ...
                        <p class="mt-1 text-sm text-slate-500">Adicione itens para definir o valor da OS.</p>
                        @can('update', $this->order)
                            <button type="button" wire:click="add"
